...
from tests.conftest import complete_profile, register_and_verify
...
async def test_voice_prompt_rejects_someone_elses_asset(client, verified, fake_storage):
    ...
    just exist."""
    from tests.conftest import register_and_verify, upload_media

    owner = await register_and_verify(client, "owner")
...
async def test_completeness_lists_what_is_missing(client, fake_storage):
    account = await register_and_verify(client, "incomplete")
