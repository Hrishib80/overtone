...
async def onboard(client, email: str, *, visible_as: list[str], interested_in: list[str]) -> dict:
    account = await register_and_verify(client, email)
    await complete_profile(client, account["headers"], visible_as=visible_as, interested_in=interested_in)
...
@pytest.mark.asyncio
async def test_submitting_a_complete_profile_activates_the_account(client):
    result = await onboard(client, f"one@{CAMPUS_DOMAIN}", visible_as=["woman"], interested_in=["man"])
    assert result["submit"]["status"] == "active"


@pytest.mark.asyncio
async def test_incomplete_profile_cannot_be_submitted(client):
...
@pytest.mark.asyncio
async def test_unverified_account_cannot_be_submitted(client):
    from tests.conftest import register

    account = await register(client, f"unverified@{CAMPUS_DOMAIN}")
    await complete_profile(client, account["headers"], visible_as=["man"], interested_in=["woman"])
    response = await client.post("/api/profile/submit", headers=account["headers"])
...
@pytest.mark.asyncio
async def test_third_person_in_a_full_segment_is_waitlisted(client):
    for i in range(2):
        result = await onboard(client, f"w{i}@{CAMPUS_DOMAIN}", visible_as=["woman"], interested_in=["man"])
        assert result["submit"]["status"] == "active"

    third = await onboard(client, f"w2@{CAMPUS_DOMAIN}", visible_as=["woman"], interested_in=["man"])
...
@pytest.mark.asyncio
async def test_caps_are_independent_per_segment(client):
    """A full women's segment must not block men from joining."""
    for i in range(2):
        await onboard(client, f"f{i}@{CAMPUS_DOMAIN}", visible_as=["woman"], interested_in=["man"])

    overflow = await onboard(client, f"f2@{CAMPUS_DOMAIN}", visible_as=["woman"], interested_in=["man"])
    assert overflow["submit"]["status"] == "waitlisted"

    man = await onboard(client, f"m0@{CAMPUS_DOMAIN}", visible_as=["man"], interested_in=["woman"])
    assert man["submit"]["status"] == "active"


@pytest.mark.asyncio
async def test_uncapped_segment_always_admits(client):
    """No cap row for nonbinary in the fixture, so it is uncapped."""
    for i in range(3):
        result = await onboard(
            client, f"nb{i}@{CAMPUS_DOMAIN}", visible_as=["nonbinary"], interested_in=["nonbinary"]
        )
        assert result["submit"]["status"] == "active"


@pytest.mark.asyncio
async def test_waitlist_position_counts_only_the_same_segment(client):
    for i in range(2):
        await onboard(client, f"a{i}@{CAMPUS_DOMAIN}", visible_as=["woman"], interested_in=["man"])
        await onboard(client, f"b{i}@{CAMPUS_DOMAIN}", visible_as=["man"], interested_in=["woman"])

    waiting_woman = await onboard(client, f"a9@{CAMPUS_DOMAIN}", visible_as=["woman"], interested_in=["man"])
    waiting_man = await onboard(client, f"b9@{CAMPUS_DOMAIN}", visible_as=["man"], interested_in=["woman"])
...
@pytest.mark.asyncio
async def test_me_reports_waitlist_position(client):
    for i in range(2):
        await onboard(client, f"c{i}@{CAMPUS_DOMAIN}", visible_as=["woman"], interested_in=["man"])
    waiting = await onboard(client, f"c9@{CAMPUS_DOMAIN}", visible_as=["woman"], interested_in=["man"])
...
@pytest.mark.asyncio
async def test_submitting_twice_is_harmless(client):
...
@pytest.mark.asyncio
async def test_a_freed_slot_is_offered_with_an_expiry(client, db_sessionmaker, scope):
    """invite_next must set a claim deadline, or a freed slot sits dead forever."""
    for i in range(2):
        await onboard(client, f"d{i}@{CAMPUS_DOMAIN}", visible_as=["woman"], interested_in=["man"])
    await onboard(client, f"d9@{CAMPUS_DOMAIN}", visible_as=["woman"], interested_in=["man"])
