...
from backend.database import ChatMessage, ChatStatus, MatchRecord, User, get_db
...
async def mark_read(match_id: str, user_id: str = Depends(require_auth), db: AsyncSession = Depends(get_db)):
    # Membership check: without it, any authenticated user could mark any
    # conversation's messages read by guessing a match id.
    match_res = await db.execute(
        select(MatchRecord)
        .where(MatchRecord.id == match_id)
        .where(or_(MatchRecord.user_a_id == user_id, MatchRecord.user_b_id == user_id))
    )
    if match_res.scalars().first() is None:
        raise HTTPException(status_code=403, detail="Not authorized")
...
async def get_conversations(user_id: str = Depends(require_auth), db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(MatchRecord).where(or_(MatchRecord.user_a_id == user_id, MatchRecord.user_b_id == user_id))
    )
        ...
        conversations.append(
            {
                ...
                "displayName": peer.display_name if peer else "Unknown",
                "lastMessageTime": (
                    last_msg.created_at.timestamp() * 1000
                    if last_msg and last_msg.created_at
                    else match.created_at.timestamp() * 1000
                ),
